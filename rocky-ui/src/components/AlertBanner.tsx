"use client";

import { TriangleAlert } from "lucide-react";
import type { SystemAlert } from "../hooks/useRocky";

export function AlertBanner({ alert }: { alert: SystemAlert | null }) {
  if (!alert || !alert.message) return null;

  return (
    <div
      role="alert"
      className="rocky-rise flex items-start gap-3 border-l-2 border-ink bg-panel px-3 py-2"
    >
      <TriangleAlert className="mt-0.5 shrink-0 text-ink" size={18} aria-hidden />
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest text-ink">
          Alerta {alert.resource ? `de ${alert.resource}` : "del sistema"}
        </p>
        <p className="mt-1 text-sm text-ink">{alert.message}</p>
      </div>
    </div>
  );
}
